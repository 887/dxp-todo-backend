...
pub async fn run(
    ...
    loop {
        let lib_reloaded_ready = lib_ready_to_reload(
            ...
            &tx_shutdown_server,
            &block_reloads_mutex,
            || hot::hot_heart::subscribe().wait_for_reload(),
        );
        ...
        let migration_reload_ready = lib_ready_to_reload(
            ...
            &tx_shutdown_server,
            &block_reloads_mutex,
            || hot::hot_migration_runner::subscribe().wait_for_reload(),
        );

        let observe_lib_hot = observe_lib(
            "tx_lib_reloaded_hot",
            || hot::hot_heart::subscribe().wait_for_about_to_reload(),
            &tx_lib_reloaded,
        );

        let observe_lib_migration = observe_lib(
            "tx_lib_reloaded_migration",
            || hot::hot_migration_runner::subscribe().wait_for_about_to_reload(),
